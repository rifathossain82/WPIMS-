...
import 'package:wpims/pages/speech/controller/model/principal_speech_model.dart';
...
class PrincipalSpeech extends StatefulWidget {

  const PrincipalSpeech({Key? key}) : super(key: key);

  @override
  State<PrincipalSpeech> createState() => _PrincipalSpeechState();
}

class _PrincipalSpeechState extends State<PrincipalSpeech> {
  late Message message;
  bool isLoading = true;
  bool isEmptyData = false;
  String statusMessage='No Data Found';
  Future getData() async {
    final response=await ApiService().get('principal-message');
    if(response.statusCode==200) {
      PrincipalSpeechApi speechInstance = principalSpeechFromJson(response.body);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: topBar(title: 'Principal Speech', isCentered: true),
